...
#Create rule to allow current client to access the SQL database
$clientIp = (Invoke-Expression -Command .\Static-Ip.ps1).Trim()
$clientFirewallRuleName = "Client_" + $ip
Write-Verbose "[Start] creating firewall rule $clientFirewallRuleName in database server $databaseServerName for IP addresses $clientIp - $clientIp"
$rule3 = New-AzureSqlDatabaseServerFirewallRule -ServerName $databaseServer.ServerName -RuleName $clientFirewallRuleName -StartIpAddress $clientIp -EndIpAddress $clientIp -Verbose
if (!$rule3) {throw "Failed to create $clientFirewallRuleName firewall rule. Failure in New-AzureSql.ps1"}
Write-Verbose "[Finish] creating $clientFirewallRuleName firewall rule in database server $databaseServerName for IP addresses $clientIp - $clientIp"
